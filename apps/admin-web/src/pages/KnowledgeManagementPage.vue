...
<template>
  <section class="admin-module knowledge-admin">
    <div class="module-heading">
      <div>
        <p class="admin-kicker">AUTHORIZED KNOWLEDGE</p>
        <h2>知识库中心</h2>
        <p>授权先于索引；检索时再次执行 ACL；引用始终绑定精确文档版本和 Chunk。</p>
      </div>
      <el-button @click="load">刷新</el-button>
    </div>
    <el-alert title="文档中的指令是不可信数据，不能授权工具调用；实时价格和可用性必须查询业务服务。" type="warning" :closable="false" show-icon />
    ...
    <el-tabs>
      <el-tab-pane label="知识空间">
        <el-table :data="spaces">
          <el-table-column prop="space_code" label="代码" />
          <el-table-column prop="name" label="名称" />
          <el-table-column prop="purpose" label="用途边界" />
      ...
      <el-tab-pane label="来源与同步">
        <el-table :data="sources">
          <el-table-column prop="source_code" label="来源" />
          <el-table-column prop="source_type" label="类型" />
          <el-table-column prop="sensitivity" label="敏感级别" />
          <el-table-column label="操作">
            <template #default="scope">
              <el-button v-if="['cms','course','activity','counseling'].includes(scope.row.source_type)" :loading="busy" @click="syncSource(scope.row)">同步公开内容</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-tab-pane>
      ...
      <el-tab-pane label="私有导入向导">
        ...
        <el-select v-model="selectedSourceId" aria-label="知识来源">
          <el-option v-for="source in sources" :key="source.id" :label="source.title" :value="source.id" />
        </el-select>
        <input aria-label="知识文档文件" type="file" accept=".pdf,.docx,.md,.txt,.html" @change="chooseFile">
        <el-input v-model="uploadCode" aria-label="文档代码" placeholder="文档代码" />
        <el-input v-model="uploadTitle" aria-label="文档标题" placeholder="内部标题" />
        <el-button type="primary" :disabled="!selectedFile || !uploadCode || !uploadTitle" :loading="busy" @click="uploadPrivateDocument">创建私有上传并校验</el-button>
      </el-tab-pane>

      <el-tab-pane label="文档与版本">
        <el-table :data="documents">
          <el-table-column prop="document_code" label="文档代码" />
          <el-table-column prop="title" label="标题" />
          <el-table-column prop="locale" label="语言" />
          <el-table-column prop="status" label="状态" />
          <el-table-column label="证据">
            <template #default="scope">
              <el-button :disabled="!scope.row.current_version_id" @click="inspectParsing(scope.row.current_version_id)">解析与溯源</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-descriptions v-if="parsingReport" title="解析报告" :column="3" border>
          <el-descriptions-item label="解析器">{{ parsingReport.parser_name }}</el-descriptions-item>
          <el-descriptions-item label="质量">{{ parsingReport.quality_score_basis_points }}</el-descriptions-item>
          <el-descriptions-item label="人工复核">{{ parsingReport.requires_manual_review ? "需要" : "已满足阈值" }}</el-descriptions-item>
        </el-descriptions>
        <article v-for="block in parsedBlocks" :key="block.block_id" class="result-card">
      ...
      <el-tab-pane label="授权治理">
        <el-table :data="authorizations">
          <el-table-column prop="rights_holder_name" label="权利方" />
          <el-table-column prop="authorization_basis" label="依据" />
          <el-table-column prop="citation_permission" label="引用权限" />
          <el-table-column prop="status" label="状态" />
          <el-table-column prop="valid_until" label="到期" />
        </el-table>
      </el-tab-pane>

      <el-tab-pane label="混合检索调试">
        <el-select v-model="selectedSpaceCode" aria-label="检索知识空间">
          <el-option
        ...
        <el-input v-model="query" aria-label="检索问题" />
        <el-select v-model="locale" aria-label="检索语言">
          <el-option label="简体中文" value="zh-CN" />
          <el-option label="繁體中文" value="zh-TW" />
          <el-option label="English" value="en" />
        </el-select>
        <el-button type="primary" @click="retrieve">运行授权检索</el-button>
        <article v-for="item in results" :key="item.chunk_id" class="result-card">
      ...
      <el-tab-pane label="索引版本">
        <el-table :data="indexes">
          ...
          <el-table-column prop="status" label="状态" />
          <el-table-column prop="evaluation_status" label="评测" />
          <el-table-column label="受控切换">
            <template #default="scope">
              <el-button v-if="scope.row.status === 'ready_for_evaluation'" :disabled="scope.row.evaluation_status !== 'passed'" @click="changeIndex(scope.row, 'activate')">激活</el-button>
              <el-button v-if="scope.row.status === 'active' && scope.row.previous_index_id" @click="changeIndex(scope.row, 'rollback')">回滚</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-tab-pane>
      ...
      <el-tab-pane label="评测门禁">
        <p v-for="dataset in evaluations" :key="dataset.id">{{ dataset.name }}：{{ dataset.case_count }} 个案例（授权和 ACL 泄漏必须为 0）</p>
        <el-table :data="evaluationRuns">
          <el-table-column prop="index_version_id" label="索引" />
          <el-table-column prop="status" label="状态" />
          <el-table-column label="通过"><template #default="scope">{{ scope.row.passed_cases }}/{{ scope.row.total_cases }}</template></el-table-column>
          <el-table-column prop="authorization_violations" label="授权违规" />
          <el-table-column prop="acl_leakage_count" label="ACL 泄漏" />
        </el-table>
      </el-tab-pane>
      ...
      <el-tab-pane label="知识审计">
        <el-table :data="auditEvents">
          ...
          <el-table-column prop="subject_type" label="对象" />
