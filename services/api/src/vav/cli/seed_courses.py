...
async def seed_courses() -> None:
    if get_settings().environment not in {"development", "test"}:
        print("Course fixtures skipped outside development/test.")
        return
    ...
    async with session_factory() as session:
        ...
        if existing is not None:
            ...
            print("Course seed already present and curriculum snapshot refreshed.")
            return
        ...
        await session.commit()
    print("Course seed complete: catalog SKU, curriculum, private video and version")
