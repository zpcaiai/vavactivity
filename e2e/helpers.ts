...
function localPythonEnvironment(): NodeJS.ProcessEnv {
  const sourceRoot = resolve(workspaceRoot, "services/api/src");
  return {
    ...process.env,
...
function useLocalSeedRunner(): boolean {
  return process.env.VAV_E2E_SEED_MODE === "local";
}

function runApiModule(moduleName: string, ...args: string[]): string {
  if (!useLocalSeedRunner()) {
    return execFileSync(
      "docker",
      ["compose", "exec", "-T", "api", "python", "-m", moduleName, ...args],
      { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }
    ).trim();
  }
  const python = process.env.VAV_E2E_PYTHON ?? resolve(workspaceRoot, ".venv/bin/python");
  return execFileSync(python, ["-m", moduleName, ...args], {
    cwd: workspaceRoot,
...
export function seedQualityFixture() {
  runApiModule("vav.cli.seed_permissions");
  runApiModule("vav.cli.seed_quality");
}

export function seedExperienceFixture(includeTestUser = false) {
  runApiModule("vav.cli.seed_permissions");
  runApiModule("vav.cli.seed_experience");
  if (includeTestUser) {
    runApiModule("vav.cli.seed_test_user", "--confirm-insecure-test-account");
  }
}

export function seedProcessGovernanceFixture() {
  runApiModule("vav.cli.seed_permissions");
  runApiModule("vav.cli.seed_process_governance");
}

export function seedDataGovernanceFixture() {
  runApiModule("vav.cli.seed_permissions");
  runApiModule("vav.cli.seed_data_governance");
}

export function seedAdminPlatformFixture() {
  runApiModule("vav.cli.seed_permissions");
  runApiModule("vav.cli.seed_admin_platform");
}
...
export function seedRecommendationFixture() {
  ...
  if (useLocalSeedRunner()) {
    ...
    execFileSync(
      "docker",
      ["compose", "exec", "-T", "postgres", "psql", "-U", "vav", "-d", "vav", "-v", "ON_ERROR_STOP=1", "-c", resetSql],
      { stdio: "pipe" }
    );
  }
}
...
export function seedMatchmakingInteractionFixture(): string {
  ...
  runApiModule("vav.cli.seed_matchmaking_interactions");
  return apiPython([
    "import asyncio",
...
export function seedRelationshipFixture(): string {
  const existingEmail = apiPython([
    ...
    "async def main():",
    "    async with session_factory() as session:",
    "        existing = (await session.execute(text(\"SELECT u.email FROM relationship_journeys j JOIN users u ON u.id=j.user_low_id WHERE u.email LIKE 'recommendation-fixture-%@example.com' ORDER BY j.created_at DESC LIMIT 1\"))).scalar_one_or_none()",
    "        print(existing or '')",
    "asyncio.run(main())"
  ]);
...
export function seedMembershipFixture(): string {
  seedRecommendationFixture();
  runApiModule("vav.cli.seed_memberships");
...
function apiPython(script: string[]): string {
  if (useLocalSeedRunner()) {
    const python = process.env.VAV_E2E_PYTHON ?? resolve(workspaceRoot, ".venv/bin/python");
    return execFileSync(python, ["-c", script.join("\n")], {
      cwd: workspaceRoot,
      env: localPythonEnvironment(),
      encoding: "utf8",
      stdio: ["ignore", "pipe", "pipe"]
...
export function resetLoginRateLimits() {
  ...
  if (useLocalSeedRunner()) {
    apiPython([
      ...
      "async def main():",
      "    redis = get_redis()",
      "    keys = [key async for key in redis.scan_iter(match='rate:*')]",
      "    if keys:",
