...
// Route-level lazy loading keeps the initial admin shell small while preserving
// a separate, cacheable chunk for every governed operational surface.
...
const MatchmakingProfileManagementPage = () =>
  import("@/pages/MatchmakingProfileManagementPage.vue");
...
const RecommendationManagementPage = () =>
  import("@/pages/RecommendationManagementPage.vue");
const MatchmakingInteractionManagementPage = () =>
  import("@/pages/MatchmakingInteractionManagementPage.vue");
...
const legacyModuleRedirects: Record<string, string> = {
  ...
  moderation: "/admin/trust-safety/reports",
  settings: "/admin/system/feature-flags",
  ...
  routes: [
    ...
    },
    {
      ...
      children: [
        ...
        ...Object.entries(legacyModuleRedirects).map(([path, redirect]) => ({ path, redirect }))
      ]
    },
    {
      path: "/admin/403",
