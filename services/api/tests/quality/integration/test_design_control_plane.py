...
async def _operator(label: str) -> UUID:
    async with session_factory() as session:
        ...
        user = User(
            email=email,
            display_email=email,
            password_hash=PasswordHasher().hash("DesignOperator!2026"),
            status="active",
...
async def test_token_release_requires_separation_and_complete_accepted_evidence() -> None:
    ...
    async with session_factory() as session:
        release = await design_service.create_token_release(
            session,
            author,
            TokenReleaseCreate(
                token_version=version,
                manifest_checksum_sha256="a" * 64,
                generated_checksum_sha256="b" * 64,
                change_summary="Production token release with generated artifact parity.",
            ),
        )
        ...
        evidence = {
            name: {"status": "accepted", "checksum_sha256": uuid4().hex + uuid4().hex}
            for name in (
                "token_build",
                "component_tests",
    ...
    async with session_factory() as session:
        with pytest.raises(VavError, match="manual review"):
            await design_service.create_audit(
                ...
                runner,
                AuditRunCreate(
                    audit_code=f"A11Y.AUTO.{uuid4().hex[:8].upper()}",
                    audit_type="accessibility",
                    application_code="design-system",
                    git_commit=uuid4().hex,
                    environment="ci",
                    status="technical_pass",
                    evidence_checksum_sha256="c" * 64,
                ),
            )
        audit = await design_service.create_audit(
            session,
            runner,
            AuditRunCreate(
                audit_code=f"A11Y.REVIEW.{uuid4().hex[:8].upper()}",
                audit_type="accessibility",
                application_code="design-system",
                git_commit=uuid4().hex,
                environment="ci",
        ...
        with pytest.raises(VavError, match="independent"):
            await design_service.review_audit(
                session, runner, audit["id"], "approve", "Self approval must remain forbidden."
            )
        approved = await design_service.review_audit(
            session,
            reviewer,
            audit["id"],
            "approve",
            "Keyboard and assistive technology evidence reviewed.",
        )
        assert approved["status"] == "approved"
...
async def test_visual_baseline_requires_independent_approval() -> None:
    ...
    async with session_factory() as session:
        baseline = await design_service.create_baseline(
            session,
            author,
            BaselineCreate(
                baseline_code=f"design-system.home.{uuid4().hex[:8]}",
                application_code="design-system",
                route_path="/",
                ...
                locale="zh-CN",
                density="comfortable",
                artifact_reference="artifact://synthetic/design-system-home.png",
        ...
        with pytest.raises(VavError, match="independent"):
            await design_service.decide_baseline(
                ...
                baseline["id"],
                "approve",
                "The author cannot approve their own baseline.",
            )
        approved = await design_service.decide_baseline(
            session,
            reviewer,
            baseline["id"],
            "approve",
            "Visual content and synthetic fixture policy reviewed.",
        )
        assert approved["status"] == "approved"
